...
**c**
```{python}
#map the value to all zipcodes
zips_texas_centroids = zips_texas_centroids.assign(mean_distance=mean_distance_mi)
```
...
texas_pos_data_inactive = pos[(pos['STATE_CD']== 'TX') & (pos['ever_inactive']== 1)]
#Grouping this by zipcode 
texas_pos_data_group = texas_pos_data_inactive.groupby('ZIP_CD')['ever_inactive'].sum().reset_index()
...
texas_pos_data_group['zip_string'] = texas_pos_data_group['ZIP_CD'].astype(int).astype(str)

#Filtering by directly affected zipcodes 
texas_inactive_zipcodes = texas_pos_data_group['ZIP_CD'].astype(int).astype(str).tolist()
texas_census_data_inactive = texas_census_data[texas_census_data['NAME'].isin(texas_inactive_zipcodes)]

#Joining the shapefile
texas_pos_data_group_map = texas_inactive_zipcodes.set_index("NAME").join(texas_pos_data_group.set_index('zip_string'))
texas_pos_data_group_map['ever_inactive'] = texas_pos_data_group_map['ever_inactive'].fillna(0)

#Creating the choropleth
texas_pos_data_group_map.plot(column='ever_inactive', legend=True)
...
There are 343 directly affected zipcodes.
...
There are 1857 zipcodes, and since there are 343 directly affected areas included there are 1,523 indirectly affected zipcodes. 
